(ns mutate
  ...
  ## Two deliberate differences from the `kotoba-lang/iso3166` copy

  This started as that copy byte-for-byte (md5 a06eda8c269efc3b7676ee04baad1ff1)
  — the interrupt-safe one. Everything above is unchanged.

  1. THE RUNTIME IS A PARAMETER. `MUTATE_CMD` selects the suite; it defaults
     to `clojure -M:test`. The point of this repo's change was portability, so
     a harness that could only drive the JVM would leave the `:cljs` side of
  ...
  2. `maxBuffer` IS RAISED to 64 MiB. `execSync`'s 1 MiB default SIGTERMs the
     child when a mutation produces a lot of output, and a SIGTERMed child is
     read by the code above as \"the process group was signalled\", which
     ABORTS THE WHOLE TABLE — so the cap does not lose one row, it loses every
     row after it."
...
(def ^:private exec-opts
  ;; See difference 2 in the namespace docstring: the 1 MiB default turns a
...
(defn -main [& args]
  ...
  (let [table (edn/read-string (.readFileSync fs "tools/mutations.edn" "utf8"))
    ...
    (when-let [sig (:signal baseline)]
      ...
      (js/process.exit (signal-exit-code sig)))
    (println "baseline:" (summary-line (:out baseline)) (str "[" test-cmd "]")
