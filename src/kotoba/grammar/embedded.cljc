...
(def catalog
 ...
 {:bytes {:desugars-to "canonical bounded empty :bytes value"
  ...
  ;;
  ;; `rel`, `query` and `handle` are the other three heads of the ADR's set
  ;; and are NOT here. They have no existing primitive to desugar onto -- a
  ;; relational value model and an effect handler respectively -- and a head
  ;; with no primitive cannot be desugared to existing primitives. They are
  ;; refused `operation has no admitted lowering`, and
  ;; `lang/surface-status.edn` `:other-gaps :pure-s-expression-core`
  ;; `:not-admitted` says why for each.
  ...
  {:desugars-to "each head onto the existing primitive it spells: lam -> the fn lambda lift, app -> ordinary prefix application, ref -> the named symbol, perform -> cap-call"
   ...
   {lam "(lam [params] body) -> (fn [params] body). One clause; the fn ABI's arity bound applies unchanged."
    ...
    perform "(perform :kind/op v ...) -> (cap-call :kind/op v ...). Carries exactly cap-call's authority: same registry resolution, same effect row, same :named-operations, same declare-then-check against the namespace's :capabilities. The numeric wire-id form is NOT admitted -- wire ids are ABI, not source vocabulary."}
